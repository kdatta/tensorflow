...
// reference inputs.
// --------------------------------------------------------------------------
//                      Eigen Concat NoCopy Op
// --------------------------------------------------------------------------
template <typename Device, typename T, AxisArgumentName AxisArgName>
class EigenConcatNoCopyBaseOp : public OpKernel {
 public:
  typedef std::vector<std::unique_ptr<typename TTypes<T, 2>::ConstMatrix>>
      ConstMatrixVector;

  explicit EigenConcatNoCopyBaseOp(OpKernelConstruction* c) : OpKernel(c) {}
  ...
  void Compute(OpKernelContext* c, const std::vector<Tensor>& values,
    ...
    const TensorShape& input_shape = input_shap es[0];
    ...
    //  int64 output_dim1 = output->NumElements() / inputs_flat_dim0;
      ...
      ConcatNoCopyCPU<T>(c->device(), inputs_flat, &output_flat);
    //}
  }
};
// --------------------------------------------------------------------------
...
template <typename Device, typename T, AxisArgumentName AxisArgName>
class MklConcatNoCopyOp : public OpKernel {
 private:
  TensorFormat data_format_;
  EigenConcatNoCopyBaseOp<Device, T, AxisArgName> eigen_concat_nocopy_op_;

 public:
  typedef std::vector<std::unique_ptr<typename TTypes<T, 2>::ConstMatrix>>
      ConstMatrixVector;

  explicit MklConcatNoCopyOp(OpKernelConstruction* c)
      : OpKernel(c), eigen_concat_nocopy_op_(c) {}
  ...
  void CallEigenVersion(OpKernelContext* context, const OpInputList& values,
    ...
    // Call Eigen concat.
    eigen_concat_nocopy_op_.Compute(context, converted_values, tf_input_shapes);
...
#define REGISTER_MKL_CPU(type)                                              \
  REGISTER_KERNEL_BUILDER(Name("_MklConcatNoCopy")                                \
                              .Device(DEVICE_CPU)                           \
                              .TypeConstraint<type>("T")                    \
                              .HostMemory("concat_dim")                     \
                              .Label(mkl_op_registry::kMklOpLabel),         \
                          MklConcatOp<CPUDevice, type, NAME_IS_CONCAT_DIM>) \
  REGISTER_KERNEL_BUILDER(Name("_MklConcatNoCopyV2")                              \
                              .Device(DEVICE_CPU)                           \
                              .TypeConstraint<type>("T")                    \
...
REGISTER_KERNEL_BUILDER(Name("_MklQuantizedConcatNoCopyV2")
                            .Device(DEVICE_CPU)
